...
export function CoverageMap({
  ...
  locations: Location[];
  /** The map art is always dark; only the caption below it sits on the section. */
  onDark?: boolean;
}) {
      ...
      >
        ...
        {pts.map(({ l, x, y }) => (
          <Link key={l.slug} href={`/service-areas/${l.slug}`} className="coverage-pin">
            ...
            <circle
              className="coverage-pin__dot"
              cx={x}
              cy={y}
              r={l.priority ? 5 : 3.5}
              fill={l.priority ? "#3fe0bf" : "#63c2fb"}
            />
            <text
              className="coverage-pin__label"
              x={x}
              y={y - 9}
              textAnchor="middle"
              fontSize="9.5"
